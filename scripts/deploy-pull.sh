#!/usr/bin/env bash
# bps-2api 拉取式部署：镜像由 GitHub Actions 构建推到 Docker Hub，服务器只做 pull + up -d。
...
#     SSHPASS='...' bash scripts/deploy-pull.sh                  # 拉 app-latest
...
# 要固化某个版本（回滚）：把 .env 里的 BPS_APP_TAG 改成 app-<sha>。
#
...
# TAG=latest 时用滚动 tag；给了 sha 就拉那一版（app-<sha>）
APP_TAG="app-${TAG}"
...
"${SSH[@]}" "$TARGET" "cd '$DEPLOY_PATH' && export BPS_APP_TAG='$APP_TAG' && \
  ${LOGIN_CMD}docker compose pull -q bps-2api && ${LOGOUT_CMD}\
  docker compose up -d && \
...
"${SSH[@]}" "$TARGET" "cd '$DEPLOY_PATH' && docker compose images bps-2api && \
  docker image inspect \$(docker inspect -f '{{.Image}}' bps-2api) -f 'bps-2api digest={{index .RepoDigests 0}}'"
...
echo "回滚：TAG=<旧 sha> 重跑本脚本，或把 $DEPLOY_PATH/.env 的 BPS_APP_TAG 换成旧 tag。"
